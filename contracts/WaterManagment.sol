...
contract WaterManagement is Ownable, AccessControl {
    ...
    mapping(address => string[]) private companyToSensortIds;
    ...
    constructor(address _waterAddress) {
    ...
    modifier onlyGovernment(address _government) {
        require(hasRole(GOVERNMENT_ROLE, _government), "Address provided hasn't got GOVERMENT role");
        _;
    }

    modifier onlyCompany(address _company) {
        require(hasRole(COMPANY_ROLE, _company), "Address provided hasn't got COMPANY role");
        _;
    }
    ...
    function addEntity (address _entityAddress, string calldata _name, string calldata _nif, string calldata _role) external onlyOwner {
        require(owner() != _entityAddress, "The entity address cannot be the same as the owner of the contract");
    ...
    function fetchEntities() external view returns(Entity[] memory ) {
        require(hasRole(GOVERNMENT_ROLE, msg.sender) || hasRole(COMPANY_ROLE, msg.sender), "The entity does not have any role");
    ...
    function createRequest(address _company) external onlyGovernment(msg.sender) returns(bool) {
        Request memory request = Request(requestId, _company,msg.sender, false, Status.PENDING);
    ...
    function answerRequest(uint _requestId, string calldata _newStatus) external onlyCompany(msg.sender) {
    ...
    function checkRequestExists(address _government,address _company) public view returns(uint) {
        require(hasRole(GOVERNMENT_ROLE, _government), "You must have a GOVERNMENT role");
    ...
    function fetchRequests() external view onlyCompany(msg.sender) returns(Request[] memory) {
        Request[] memory requests = new Request[](companyToRequestIds[msg.sender].length);
    ...
    function registerSensor(string calldata _sensorId) external onlyCompany(msg.sender) {
        companyToSensortIds[msg.sender].push(_sensorId);
    }
    ...
    function pushData(string calldata _sensorId, string calldata _siteId, uint _value, uint _timestamp) external onlyCompany(msg.sender) returns(bool) {
        ...
        require(checkSensorIdToCompany(_sensorId, msg.sender), "This sensor ID is not registered");
        require(fetchBenchmark(_siteId, msg.sender) != 0, "This site ID is not registered" );
    ...
    function checkSensorIdToCompany(string calldata _sensorId, address _company) private view returns(bool) {
        string[] memory ids = companyToSensortIds[_company];
        for (uint i = 0; i < ids.length; i++) 
        {
    ...
    function pullDataByGovernment(address _company) external view onlyGovernment(msg.sender) returns(SensorData[] memory) {
        require(checkRequestApproval(msg.sender, _company), "You are not allowed to request this data");
